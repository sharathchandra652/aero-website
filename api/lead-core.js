/**
 * Aero Villas — Shared lead logic (runtime agnostic)
 *
 * Pure helpers used by both the local Express handler (api/leads.js)
 * and the Cloudflare Worker (src/index.js). No Node-only dependencies here.
 */

const DEFAULT_SALES_EMAIL = 'sales@aerovillas.in';
...
// Helper: Generate unique Lead ID (AV-YYYYMMDD-XXXX)
...
/**
 * Validate a raw form payload and turn it into a lead record.
 * Returns { error } when validation fails, otherwise { lead }.
 */
...
// Helper: Email subject line
function buildSubject(lead) {
  return `[AERO VILLAS] New ${lead.leadType === 'schedule-visit' ? 'Visit Request' : 'Enquiry'} — ${lead.name}`;
}

// Helper: Plain-text fallback body
function buildEnquiryEmailText(lead) {
  const lines = [
    `AERO VILLAS — New Enquiry (Lead ID: ${lead.leadId})`,
    '',
    `Name: ${lead.name}`,
  ...
  if (lead.visitDate)   lines.push(`Visit Date: ${lead.visitDate}`);
  if (lead.visitTime)   lines.push(`Visit Time: ${lead.visitTime}`);
  if (lead.visitGuests) lines.push(`No. of Guests: ${lead.visitGuests}`);
  lines.push('', `Message: ${lead.message || 'No message provided.'}`, '',
    `Source: ${lead.source || 'Website'}`, `Page URL: ${lead.page || '/'}`, `Submitted: ${lead.createdAt}`);
  return lines.join('\n');
}
